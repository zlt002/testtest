...
import { createClassname } from '../../utilities/index.js'
import { MetatipStyles, metatip_css, supportsAdoptedStyleSheets } from '../styles.store.js'
...
;(() => {
  ...
  if (!registry.get('visbug-metatip')) {
    try {
      registry.define('visbug-metatip', Metatip);
    } catch (error) {
      if (!(error instanceof DOMException) || error.name !== 'NotSupportedError' || !registry.get('visbug-metatip')) {
        throw error;
      }
    }
  }
