import { BoxModelStyles, boxmodel_css, supportsAdoptedStyleSheets } from '../styles.store.js'
...
;(() => {
  ...
  if (!registry.get('visbug-boxmodel')) {
    try {
      registry.define('visbug-boxmodel', BoxModel);
    } catch (error) {
      if (!(error instanceof DOMException) || error.name !== 'NotSupportedError' || !registry.get('visbug-boxmodel')) {
        throw error;
      }
    }
  }
