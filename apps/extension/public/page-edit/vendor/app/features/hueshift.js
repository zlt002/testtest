...
export function HueShift({Color, Visbug}) {
  const state = {
    active_color: Color.getActive(),
    elements: [],
  }

  const syncSelectedElements = elements => {
    state.elements = elements
    Color.setActive(state.active_color)
  }

  Visbug.onSelectedUpdate(syncSelectedElements)
  ...
  hotkeys(key_events, (e, handler) => {
    ...
    Visbug.recordStyleMutation({
      elements: selectedNodes,
      label: 'hue-shift',
      mutate: () => keys.includes('left') || keys.includes('right')
  ...
  return () => {
    Visbug.removeSelectedCallback(syncSelectedElements)
